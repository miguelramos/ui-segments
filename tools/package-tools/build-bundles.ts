...
export class PackageBundler {
  ...
  /** Bundles the primary entry-point w/ given entry file, e.g. @Segment/cdk */
  ...
  /** Bundles a single secondary entry-point w/ given entry file, e.g. @Segment/cdk/a11y */
  ...
  private async createRollupBundle(config: RollupBundleConfig) {
    ...
    const writeOptions = {
      ...
      moduleId: '',
      moduleName: config.moduleName || 'ng.segment',
    ...
    if (config.format === 'umd') {
      ...
      if (this.buildPackage.exportsSecondaryEntryPointsAtRoot &&
        ...
        const importRegex = new RegExp(`@segment/${this.buildPackage.name}/.+`);
        external = external.filter(e => !importRegex.test(e));

        // Use the rollup-alias plugin to map imports of the form `@Segment/material/button`
  ...
  /**
   * Gets mapping of import aliases (e.g. `@Segment/material/button`) to the path of the es5
   * bundle output.
  ...
  private getResolvedSecondaryEntryPointImportPaths(bundleOutputDir: string) {
    return this.buildPackage.secondaryEntryPoints.reduce((map, p) => {
      map[`@segment/${this.buildPackage.name}/${p}`] =
