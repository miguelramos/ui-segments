...
const cdkSecondaryEntryPoints = getSubdirectoryNames(join(buildConfig.packagesDir, 'carbon'));

/** List of potential secondary entry-points for the segment package. */
const matSecondaryEntryPoints = getSubdirectoryNames(join(buildConfig.packagesDir, 'ui'));
...
const rollupCdkEntryPoints = cdkSecondaryEntryPoints.reduce((globals: any, entryPoint: string) => {
  globals[`@segment/carbon/${entryPoint}`] = `ng.carbon.${dashCaseToCamelCase(entryPoint)}`;
  return globals;
}, {});
...
const rollupMatEntryPoints = matSecondaryEntryPoints.reduce((globals: any, entryPoint: string) => {
  globals[`@segment/ui/${entryPoint}`] = `ng.segment.${dashCaseToCamelCase(entryPoint)}`;
  return globals;
}, {});
...
export const rollupGlobals = {
  ...
  '@segment/ui': 'ng.segment',
  '@segment/carbon': 'ng.carbon',
